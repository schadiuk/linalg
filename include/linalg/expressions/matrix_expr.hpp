...
namespace linalg {
    ...
    struct MatRef : MatExpr<MatRef<T, L>> {
        ...
        /// @brief Elementwise-classification opt-in.
        /// @note If element (i,j) depends only on the same cell of `mat`, `depends_on`-triggered aliasing never needs a temp buffer for this leaf.
        static constexpr bool is_elementwise = true;

	    /// @brief Constructor.
	    /// @param m The reference to be assigned.
	    explicit MatRef(const Matrix<T, L>& m) noexcept : mat(m) {};

	    size_t rows() const noexcept { return mat.rows(); };
	    size_t cols() const noexcept { return mat.cols(); };
        size_t stride() const noexcept { return mat.stride(); };
	    ...
	    bool depends_on(const void* p, size_t bytes) const noexcept { return mat.depends_on(p, bytes); };
    };
    ...
    struct GemmExpr : MatExpr<GemmExpr<E1, E2>> {
        detail::operand_t<E1> a;
        detail::operand_t<E2> b;

        // NOT elementwise: cell (i,j) reduces over an entire row of `a` and column of `b`.
